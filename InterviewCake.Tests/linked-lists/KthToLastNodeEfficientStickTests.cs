...
{
    public class KthToLastNodeEfficientStickTests
    {
        [Fact]
        public void FirstToLastNodeTest()
        {
            ...
            var k = 1;
            var actual = KthToLastNode.GetNodeByEfficientStickMethod(k, listNodes[0]);
        ...
        public void KIsGreaterThanLinkedListLengthTest()
        {
            var listNodes = ValuesToLinkedListNodes(new int[] { 1, 2, 3, 4 });
            var k = 5;
            Assert.Throws<ArgumentException>(() => KthToLastNode.GetNodeByEfficientStickMethod(k, listNodes[0]));
        }
